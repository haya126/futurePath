    ...
    for name, data, final_score in matched:
        ...
        if "paths" in data and data["paths"]:
            ...
            for p in data["paths"]:
                if isinstance(p, dict):
                    # لون يعتمد على إذا الطالب مستوفي الحد الأدنى
                    color = "green" if final_score >= p.get("min_score", 0) else "red"

                    # جلب سنوات المسار إذا موجودة
                    years_text = f" — {p['years']} سنوات" if "years" in p else ""

                    paths_html += f"""
                        <li style='color:{color};'>
                            {p['name']} (الحد الأدنى: {p['min_score']}%) {years_text}
                        </li>
                    """
                else:
        ...
        st.markdown(f"""
            <div style='border-right: 6px solid #4F7678; padding: 20px 25px; margin: 20px 0; background-color: #f9f9f9; border-radius: 10px; text-align: right;'>
                ...
                <p><strong>سنوات الدراسة (عام):</strong> {data['years']} سنوات</p>
                {paths_html}
            </div>
        """, unsafe_allow_html=True)

    # ملاحظة تظهر مرة واحدة فقط
    st.markdown("""
